...
def not_bad(s):
    if  s.find('not') < s.find('bad'):
        return s[:s.find('not')] + "good" + s[s.find('bad')+3:len(s)]
    else:
        return s    
...
def front_back(a, b):
    if len(a)%2==0 and len(b)%2==0:
        return a[:len(a)//2] + b[:len(b)//2] + a[len(a)//2:len(a)] + b[len(b)//2:len(b)]
    elif len(a)%2==0 and len(b)%2==1:
        return a[:len(a)//2] + b[:len(b)//2+1] + a[len(a)//2:len(a)] + b[len(b)//2+1:len(b)]
    elif len(a)%2==1 and len(b)%2==0: 
        return a[:len(a)//2+1] + b[:len(b)//2] + a[len(a)//2+1:len(a)] + b[len(b)//2:len(b)]
    elif len(a)%2==1 and len(b)%2==1: 
        return a[:len(a)//2+1] + b[:len(b)//2+1] + a[len(a)//2+1:len(a)+1] + b[len(b)//2+1:len(b)]      
